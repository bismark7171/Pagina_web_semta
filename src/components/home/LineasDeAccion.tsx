    ...
    <section id="lineas-de-accion" className="bg-background py-20">
      <div className="mx-auto w-full max-w-container-semta px-gutter-desktop">
          ...
          <div className="flex flex-col items-start justify-between gap-6 md:flex-row md:items-end">
            <div className="max-w-xl">
              ...
              <h2 className="mt-4 font-headline-xl text-on-surface">
                Líneas de Acción que <span className="text-primary">transforman territorios</span>
              </h2>
              <p className="mt-3 text-body-lg text-on-surface-variant">
                Cinco ejes estratégicos articulados con las realidades del Altiplano y los Valles de
                Bolivia.
              </p>
            </div>
            <Link
        ...
        <div className="mt-12 grid gap-6 lg:grid-cols-5">
          <FadeIn className="lg:col-span-3 lg:row-span-2" delay={0.05}>
            <article
              id={featured.slug}
              className="group relative h-full overflow-hidden rounded-3xl bg-primary-container p-8 text-on-primary-container lg:p-10"
            >
              <MaterialIcon
                name="water"
              ...
              <h3 className="mt-6 font-headline-lg text-[26px] leading-tight">{featured.titulo}</h3>
          ...
          {rest.map((l, i) => (
            <FadeIn
              key={l.titulo}
              className={i === 3 ? "lg:col-span-2" : "lg:col-span-1"}
              delay={0.1 + i * 0.08}
            >
              <Link
                href="/proyectos"
                id={l.slug}
                className="group flex h-full flex-col overflow-hidden rounded-3xl border border-outline-variant bg-surface-container-lowest p-8 transition-all duration-300 hover:-translate-y-1 hover:shadow-card-semta hover:border-primary/30"
              >
                ...
                <h3 className="mt-6 font-headline-md text-on-surface">{l.titulo}</h3>
                ...
                <p className="mt-6 flex items-center gap-2 border-t border-outline-variant pt-4 font-label-md font-semibold text-primary">
                  ...
                  <MaterialIcon
                    name="arrow_forward"
                    className="text-[1em] transition-transform group-hover:translate-x-0.5"
                  />
                </p>
              </Link>
            </FadeIn>
