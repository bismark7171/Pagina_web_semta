    ...
    <section className="bg-background py-20">
      <div className="mx-auto w-full max-w-container-semta px-gutter-desktop">
        <div className="grid gap-6 lg:grid-cols-2">
          {callouts.map((c, i) => (
            <FadeIn key={c.titulo} delay={i * 0.12}>
              <article
                className={cn(
                  "relative flex h-full flex-col overflow-hidden rounded-3xl p-8 lg:p-10",
                  fondoPorIndex[i % 2],
                )}
              >
                <MaterialIcon
                ...
                <h3 className="mt-6 max-w-md font-headline-lg leading-tight">{c.titulo}</h3>
                ...
                <div className="mt-8 flex flex-wrap gap-3">
                  {c.botones.map((btn) => (
                    <Button
                      ...
                      className={cn(
                        btn.tipo === "primary" ? "bg-bosque text-white hover:bg-bosque/90" : "",
                      )}
                    >
